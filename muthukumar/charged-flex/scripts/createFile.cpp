...
int create_data_file()
{
	...
	out<<Npolym*Nchain+Ncation+Nanion<<" atoms\n";
...
{	
	...
	for(i=0;i<3;i++)
		out<<tmp[i]<<" ";
	out<<"\n#Pair definition\n";
	out<<"special_bonds\tlj 0 1 1 \t coul 0 1 1\n";
	in>>str>>tmp[0];
	if(res)
	...
	out<<"pair_style\t\t lj/cut/coul/long "<<tmp[0]<<" "<<tmp[1]<<"\n";
...
*/
	//Number of polymer chain.. convert from mole/L to molecule/nm^3 then multiply by box in nm^3
		...
		for(i=0;i<3;i++)
			tmp*=(box_max[i]-box_min[i])*0.23; //0.23 nm is the distance between consecutive charges on the polymer
		Npolym	=ceil(tmp);
